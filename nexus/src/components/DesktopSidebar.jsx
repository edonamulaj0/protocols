import {
  ...
  IoInformationCircleOutline, IoLockClosedOutline
...
function navActive({ isActive }) {
  return `${linkBase} ${isActive ? 'border-l-[var(--signal)] text-[var(--text-hi)] bg-[var(--ink-800)]' : ''}`
}
...
export function DesktopSidebar({ onNewDiscussion }) {
  ...
  return (
    <aside
      className="fixed bottom-0 left-0 top-0 z-[55] hidden w-[60px] flex-col border-r border-[var(--border)] bg-[var(--ink-950)] md:flex xl:w-[220px]"
      aria-label="Main navigation"
    >
      {/* Masthead — full on desktop, icon-only on tablet */}
      ...
      <div className="border-t border-[var(--border)] p-3 space-y-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        ...
        <button
          ...
          className="signal-glow-hover hidden w-full rounded-none border border-[var(--signal)] bg-[var(--signal)] py-2 text-[10px] font-bold uppercase tracking-[.1em] text-white transition-colors hover:bg-[var(--signal-dim)] xl:block"
        >
          Submit Topic
        </button>
        ...
        {googleSub ? (
          <button
            type="button"
            onClick={() => signOut()}
            className="hidden w-full py-1.5 text-[9px] font-semibold uppercase tracking-wide text-[var(--muted)] transition-colors hover:text-rose-300 xl:block"
          >
            Sign out
          </button>
        ) : null}
